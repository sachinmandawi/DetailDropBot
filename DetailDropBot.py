...
async def search_ifsc_info(ifsc: str) -> str:
    ...
    try:
        ...
        if isinstance(data, dict):
            ...
            # If BANK is in response, it is a success
            if data.get('BANK'):
                result = format_header("🏦", "BANK DETAILS (IFSC)") + "\n"
                result += add_field("🏛️", "Bank", data.get('BANK'), False)
                result += add_field("🌿", "Branch", data.get('BRANCH'), False)
                result += add_field("🆔", "IFSC", data.get('IFSC'))
                result += add_field("🏢", "Bank Code", data.get('BANKCODE'))
                result += add_field("🔢", "MICR", data.get('MICR'))
                ...
                if details_list:
                    result += add_field("📍", "Branch Details", ", ".join(details_list), False)
                    
                result += add_field("🗺️", "State", data.get('STATE'), False)
                result += format_separator() + "\n"
                ...
                result += add_field("🌐", "SWIFT", data.get('SWIFT'))
                result += format_separator() + "\n"
                
                result += add_field("📍", "Address", data.get('ADDRESS'), False)
...
async def ifsc_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    result = await search_ifsc_info(context.args[0].upper())
    await send_formatted_message(update, result, msg)
...
async def handle_ifsc(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = await update.message.reply_text("🔍 <b>Searching bank details...</b>", parse_mode='HTML')
    result = await search_ifsc_info(ifsc)
    await send_formatted_message(update, result, msg)
    return ConversationHandler.END
...
+------------------------------+
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            WAITING_IFSC: [MessageHandler(filters.TEXT & ~filters.COMMAND, handle_ifsc)],
        },
